                    ...
                    <div class="card shadow-xs border">
                        <div class="card-header pb-0">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Nouveau Sponsoring</h6>
                                    <p class="text-sm mb-0">Associer un sponsoring à un événement</p>
                                </div>
                        ...
                        <div class="card-body">
                            <form action="{{ route('sponsoring.store') }}" method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('type_sponsoring') is-invalid @enderror" id="type_sponsoring" name="type_sponsoring" required>
                                                ...
                                                @foreach($typesSponsorings as $type)
                                                    <option value="{{ $type->value }}" {{ old('type_sponsoring') == $type->value ? 'selected' : '' }}>
                                                        {{ $type->label() }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('type_sponsoring')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                            <small class="text-muted">Ex: Argent, Matériel, Logistique</small>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                ...
                                <div class="d-flex justify-content-end mt-4">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save me-2"></i>Créer le Sponsoring
                                    </button>
                                </div>
                            </form>
                        </div>
